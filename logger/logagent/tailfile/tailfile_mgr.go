...
type tailTaskMgr struct {
	...
	collectEntryList []*conf.CollectEntry     // 所有配置项
	confChan         chan []conf.CollectEntry // 等待新配置的通道
}

var ttMgr *tailTaskMgr

// Init 初始化
func Init(allConf []*conf.CollectEntry) (err error) {
	// allConf里面存了若干个日志的收集项,
	// 针对每一个日志收集项创建一个对应的tailObj
	ttMgr = &tailTaskMgr{
		tailTaskMap:      make(map[string]*tailTask, 20),
		collectEntryList: allConf,
		confChan:         make(chan []conf.CollectEntry), // 做一个阻塞channel
	}
...
func (t *tailTaskMgr) isExist(conf conf.CollectEntry) bool {
	_, ok := t.tailTaskMap[conf.Path]
	return ok
}

func SendNewConf(newConf []conf.CollectEntry) {
